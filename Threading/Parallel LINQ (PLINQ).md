...
var parallelResult2 .rs.AsParallel().WithExecutionMode(ParallelExecutionMode.ForceParallelism).Where(i => i % 2 == 0).ToArray();
...
var parallelResult3 = numbers.AsParallel().WithDegreeOfParallelism(2).Where(i => i % 2 == 0).ToArray();
...
Console.WriteLine("the above results are returned in any order. If you want to sort the results then use the AsOrdered operator");
var parallelResult4 = numbers.AsParallel().AsOrdered().Where(i => i % 2 == 0).ToArray();
...
Console.WriteLine("AsSequential() is used to change a subcomponent of a query from parallel to sequential");
var parallelResult5 = numbers.AsParallel().AsOrdered().Where(i => i % 2 == 0).AsSequential().Take(2);
...
Console.WriteLine("ForAll() allows parallel iteration of a collection but ignores any ordering");
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbODk4MDYyODczXX0=
-->